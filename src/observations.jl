"""
    ...
    observations(
        offsets::Vector{<:DateOffset},
        horizon::Horizon,
        window::AbstractVector{<:StepRange},
        bid_time::T,
    ) where T<:Union{ZonedDateTime, LaxZonedDateTime} -> (Vector{T}, Vector{T}, Matrix{T})
...
If given a `window`, or collection of `window`s, the `sim_now`s are generated by stepping
back from `bid_time` by each entry in the `window`(s).
...
function observations(
    offsets::Vector,
    horizon::Horizon,
    windows::AbstractVector{<:StepRange},
    bid_time::NowType,
)
    # this combines all the sim_nows together, all the targets together, etc.
    return vcat.(map(w -> observations(offsets, horizon, w, bid_time), windows)...)
end
